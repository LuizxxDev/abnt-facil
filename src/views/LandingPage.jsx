...
const LandingPage = () => {
    ...
    const featuresData = [
        {
            ...
            desc: "Arraste e solte capítulos facilmente. Estruture o seu referencial teórico sem confusão com o nosso sistema de blocos."
        },
        {
            icon: <Zap size={24} className="text-blue-400" />,
            title: "Gerador NBR 6023",
            desc: "Chega de sofrer. Preencha os dados e nós geramos a citação perfeita em menos tempo do que leva a tirar um café expresso."
        },
        {
            icon: <Download size={24} className="text-purple-400" />,
            title: "Exportação PDF Exata",
            desc: "O seu trabalho formatado num único clique. Vá buscar um café enquanto a magia acontece (spoiler: é instantâneo)."
        },
        {
            icon: <ShieldCheck size={24} className="text-orange-400" />,
            title: "Privacidade Total",
            desc: "Tudo é salvo no seu navegador ou na sua conta segura na nuvem. O seu TCC é totalmente privado e seguro."
        }
    ];
    ...
    const faqData = [
        ...
        },
        {
            ...
            answer: "Recomendamos que faça login com a sua conta Google para sincronização automática na nuvem. Se preferir usar sem login, utilize a função 'Exportar Backup' no Dashboard regularmente para salvar os seus ficheiros no computador."
        }
    ];
    ...
    return (
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-green-500/30 overflow-x-hidden transition-colors duration-500">
            ...
            <main className="relative z-10 max-w-6xl mx-auto px-6 py-12 md:py-20 space-y-24 md:space-y-32">
                ...
                {/* Sobre e Apoio Financeiro (Tema Café Forte) */}
                ...
                <section className="py-10 max-w-4xl mx-auto">
                    ...
                    <div className="grid gap-4">
                        {faqData.map((item, index) => (
                            <div 
                                key={index} 
                                className={`border transition-all duration-300 rounded-2xl overflow-hidden ${openFaq === index ? 'bg-slate-900 border-emerald-500/40 shadow-lg shadow-emerald-900/10' : 'bg-slate-900/40 border-slate-800 hover:border-slate-700'}`}
                            >
                                <button 
                                    onClick={() => setOpenFaq(openFaq === index ? null : index)}
                                    className="w-full px-6 py-5 md:py-6 flex items-center justify-between text-left focus:outline-none"
                                >
                                    <span className={`font-bold text-sm md:text-base ${openFaq === index ? 'text-emerald-400' : 'text-slate-200'}`}>
                                        {item.question}
                                    </span>
                                    <div className={`p-1 rounded-full transition-colors ${openFaq === index ? 'bg-emerald-500/10' : 'bg-slate-800'}`}>
                                        <ChevronDown 
                                            size={20} 
                                            className={`transition-transform duration-300 ${openFaq === index ? 'rotate-180 text-emerald-500' : 'text-slate-500'}`} 
                                        />
                                    </div>
                                </button>
                                <div className={`transition-all duration-300 ease-in-out ${openFaq === index ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="px-6 pb-6 md:pb-8 text-sm text-slate-400 leading-relaxed border-t border-slate-800/50 pt-5">
                                        {item.answer}
                                    </div>
                                </div>
                            </div>
            ...
            {/* MODAL DE FEEDBACK */}
            {showFeedback && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#020617]/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-700 rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center p-6 border-b border-slate-800">
                            <h3 className="text-xl font-black text-white flex items-center gap-2">
                                <MessageSquare size={20} className="text-blue-500"/> Enviar Feedback
                            </h3>
                            <button onClick={() => setShowFeedback(false)} className="text-slate-500 hover:text-white transition-colors bg-slate-800/50 hover:bg-slate-800 p-2 rounded-full focus:outline-none">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-6 flex-1">
                            ...
                            <div className="flex gap-2 p-1.5 bg-[#020617] rounded-xl border border-slate-800">
                                <button
                                    ...
                                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-xs font-bold transition-all focus:outline-none ${feedbackType === 'bug' ? 'bg-slate-800 text-red-400 shadow-sm border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
                                >
                                    <Bug size={16}/> Reportar Bug
                                </button>
                                <button
                                    onClick={() => setFeedbackType('suggestion')}
                                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-xs font-bold transition-all focus:outline-none ${feedbackType === 'suggestion' ? 'bg-slate-800 text-emerald-400 shadow-sm border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
                                >
                        ...
                        <div className="p-6 border-t border-slate-800 bg-slate-900/50 flex justify-end gap-3">
                            <button onClick={() => setShowFeedback(false)} disabled={isSubmitting} className="px-5 py-3 text-sm font-bold text-slate-400 hover:text-white transition-colors focus:outline-none disabled:opacity-50">
                                Cancelar
                            </button>
                            <button 
