...
const SortableNavItem = ({ item, scrollToSection, settings }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`w-full text-left p-1.5 rounded-lg text-xs transition-all flex items-center gap-1 group ${settings.theme === 'dark' ? 'hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'hover:bg-slate-50 text-slate-500 hover:text-slate-800'}`}>
            <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 text-slate-300 hover:text-green-500 transition-colors touch-none" title="Arrastar para reordenar">
                <GripVertical size={14} />
            </div>
            <button onClick={() => scrollToSection(item.id)} className="flex items-center gap-2 flex-1 truncate text-left outline-none">
...
const Sidebar = ({ sumarioItens, scrollToSection, setShowOutline, settings, onReorder }) => {
    ...
    return (
        <aside className={`w-60 border-l hidden xl:flex flex-col print:hidden transition-all ${settings.theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={sumarioItens.map(i => i.id)} strategy={verticalListSortingStrategy}>
                    {sumarioItens.map(item => (
                        <SortableNavItem key={item.id} item={item} scrollToSection={scrollToSection} settings={settings} />
                    ))}
                </SortableContext>
              </DndContext>
